/* 客群基本信息：综合报告 / 客群矩阵 / 年龄性别 / 时段高峰 / 客流趋势
   数据源：后端 /api/customer/*（MySQL 真实聚合，含 source 口径标注）。
   图表用 ECharts；客流趋势带次日预测（客户端线性回归）。 */
(function () {
  ...
  function getStoreId() {
    if (window.getStoreId) return window.getStoreId();
    const sel = document.getElementById("globalStore");
    return sel ? sel.value : "";
  }
  function getPeriod() {
    return {
      start: document.getElementById("customerStartDate")?.value || "",
      end: document.getElementById("customerEndDate")?.value || "",
    };
  }

  /** 初始化日期范围（来自真实数据 min/max）；切换门店时重置默认最近 7 天 */
  async function initDateRange() {
    ...
    if (!storeId || !sEl || !eEl) return;
    try {
      const data = await FenqunAPI.api("/customer/range?storeId=" + encodeURIComponent(storeId));
      range = data.range;
      if (range && range.lo && range.hi) {
        sEl.min = range.lo; sEl.max = range.hi; eEl.min = range.lo; eEl.max = range.hi;
        const d7 = new Date(range.hi + "T00:00:00");
        d7.setDate(d7.getDate() - 6);
        sEl.value = FenqunAPI.fmtDate(d7);
        eEl.value = range.hi;
      }
    } catch (_) { /* 无数据时保持默认 */ }
  }
  ...
  async function fetchCustomer(path) {
    ...
    let q = "/customer/" + path + "?storeId=" + encodeURIComponent(storeId);
    if (start) q += "&start=" + start;
    if (end) q += "&end=" + end;
    return FenqunAPI.api(q);
  }

  function sourceNote(st) {
    const note = st?.source?.personaNote;
    if (!note) return "";
    return `<p class="muted" style="margin-top:10px;font-size:.78rem">数据口径：${note}</p>`;
  }
  ...
  function predictNext(daily, personaAgg) {
    const total = linearPredict(daily.slice(-14).map((d) => d.passers));
    const totalPersona = PERSONAS.reduce((s, p) => s + (personaAgg[p]?.passers || 0), 0) || 1;
    const byPersona = PERSONAS
      .map((p) => ({ name: p, count: Math.round(total * ((personaAgg[p]?.passers || 0) / totalPersona)), pct: Math.round(((personaAgg[p]?.passers || 0) / totalPersona) * 1000) / 10 }))
  ...
  async function renderReport() {
    const st = await fetchCustomer("report");
    const noData = !st.totalPassers && !st.totalEnters;
    const emptyNote = noData
      ? `<div class="warn-box" style="margin-bottom:12px">所选时段该门店暂无客流数据（真实门店需摄像头已接入并上报，或调整日期范围查看演示门店）。</div>`
      : "";
    const pred = predictNext(st.daily, st.personaAgg);
    const nextDate = nextDateOf(st.daily[st.daily.length - 1]?.d);
    const isDay = st.daily.length <= 1;
    const isWeek = st.daily.length <= 7;

    const kpis = [
      { label: "总过店人数", value: fmt(st.totalPassers), note: isDay ? "期间路过总人次" : `日均 ${fmt(Math.round(st.totalPassers / st.nDays))} 人` },
      { label: "总进店人数", value: fmt(st.totalEnters), note: isDay ? "期间进店总人次" : `日均 ${fmt(Math.round(st.totalEnters / st.nDays))} 人` },
      { label: "进店转化率", value: pct(st.conv), note: "进店 / 过店" },
      { label: "峰值时段", value: `${st.peakHour}:00`, note: `时均过店 ${fmt(st.hourly[st.peakHour].passers)} 人` },
      { label: "客流最高人群", value: st.topPersona, note: "按画像人数计" },
    ];
    ...
    panelEl().innerHTML =
      ...
      `<div class="ci-grid-2">
        <div>${section("人群结构占比（按画像）", chartDiv("repPie", 320))}</div>
  ...
  async function renderMatrix() {
    const st = await fetchCustomer("matrix");
    ...
    panelEl().innerHTML = section("客流潜力 × 进店贡献（按人群）", chartDiv("cusMatrixChart", 380) +
    ...
    renderChart("cusMatrixChart", {
      ...
      yAxis: { name: "进店贡献", max: 100, min: 0, splitLine: { lineStyle: { type: "dashed" } } },
  ...
  async function renderAgeSex() {
    const st = await fetchCustomer("age-sex");
    const persons = st.persons.map((p, i) => ({ name: p.name, value: p.passers, color: PERSONA_COLORS[i], enters: p.enters }));
    panelEl().innerHTML = `<div class="ci-grid-2">
      <div>${section("人群结构占比（按画像）", chartDiv("cusPie", 330))}</div>
      <div>${section("各人群过店 / 进店", chartDiv("cusBar", 330))}</div></div>` + sourceNote(st);
  ...
  async function renderTimePeak() {
    const st = await fetchCustomer("time-peak");
    const nDays = st.nDays;
    const matrix = Array.from({ length: 24 }, (_, h) => {
      const m = {}; PERSONAS.forEach((p) => { m[p] = st.personaByHour[p]?.[h] || 0; }); return m;
    });
  ...
  async function renderFlowTrend() {
    const st = await fetchCustomer("flow-trend");
    const total = linearPredict(st.daily.slice(-14).map((d) => d.passers));
    const pred = {
      total,
      byPersona: st.personaShare.map((p) => ({ name: p.name, count: Math.round(total * (p.pct || 0) / 100), pct: p.pct })),
    };
    const nextDate = nextDateOf(st.daily[st.daily.length - 1]?.d);
  ...
  async function showCategory(cat) {
    ...
    try {
      ...
      el.innerHTML = `<div class="warn-box">加载失败：${e.message}（后端客群聚合接口）</div>`;
    }
  }
